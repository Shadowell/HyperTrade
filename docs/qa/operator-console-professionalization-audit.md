...
1. **P0 — durable task delivery.** Route default CLI chat through the Sprint 116
   Mission Runtime. Return the Mission id before streaming; reattach using an
   event cursor after network loss or deployment. Render the persisted final
   projection even if the live stream ends early.
...
3. **P1 — one answer contract.** Introduce `OperatorResponseV1`: `summary`,
   `market_freshness`, `evidence`, `risk_state`, `recommended_actions`,
   `artifact_refs`, and `recovery`. Rich, plain, and Web renderers consume this
   projection rather than choosing independently among raw tool payloads.
...
- Every completed task renders exactly one visible operator answer, with a
  safe evidence drill-down and an explicit next action.
