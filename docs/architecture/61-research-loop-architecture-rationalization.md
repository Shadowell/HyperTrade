...
### AVO 当前实现范围（2026-09-09）

`arc/avo.py` 实现受约束的工具循环，`arc/store.py` 提供研究执行锁，`worker.py` 领取持久 AVO 任务。
模型可以检查知识/候选、提出候选、请求开发实验、选择最终验证或主动结束；它不能调用 Paper/Live 工具。
候选由现有策略族和参数编译器生成，属于有界策略变异，不是任意 Python 程序自我改写。
开发窗口与最终窗口在创建时冻结并隔离；最终窗口一旦使用，同一任务不得继续调参。基础窗口验证尚不等于完整 V2 统计与数据质量认证。
原研究与模拟盘记录不迁移或重置；7+7 运行反馈触发仍待下一切片接入。
...
## 11. 论文驱动的可靠进化边界（2026-09-13）

本节取代“加入更多角色即可获得自主进化”的推断。目标是让策略和研究经验在外部证据下逐步改进；
权重训练、运行时自改权限/裁判/生产代码不属于当前实现。以下是论文机制与本项目采用方式的映射，
不是对论文的完整复现，也不能把通用基准提升等同于交易收益提升。

| 一手论文 | 可借鉴机制 | HyperTrade 决策与限制 |
| --- | --- | --- |
| [GEPA，2025；2026-02 v2](https://arxiv.org/abs/2507.19457v2) | 从轨迹反馈提出变异，保留多目标搜索中的互补候选 | 保留代码/参数/实验谱系，提案必须可证伪；不把单一最高收益当作搜索成功 |
| [ACE，2025](https://arxiv.org/html/2510.04618v1) | 生成、反思、整理分工；增量维护经验，避免反复摘要丢失细节 | 原始实验不改写，模型只读有界经验投影；正反例并存；整理器错误可能造成有害经验，因此不自动视为真理 |
| [MemRL，2026-01](https://arxiv.org/html/2601.03192v1) | 先匹配适用情境，再依据实际使用反馈估计记忆效用 | 先上线标的/周期/时间/身份门禁；效用学习待配对消融后启用。市场非平稳，不直接套用论文平稳奖励下的收敛论证 |
| [HarnessEvolve，2026-09-01 预印本](https://arxiv.org/html/2609.00829v1) | 参考轨迹定位错误，质量门禁与性能回归门禁分离 | 采用证据约束和新旧案例回放。交易无已知最佳答案，不能把事后最优买卖点或未来收益作为可用参考轨迹 |
| [Darwin Gödel Machine，2025](https://arxiv.org/html/2505.22954v1) | 候选归档、分支探索、实际评测后保留改进 | 独立候选与原版本并存；代码实验仅限研究边界，生产执行、预算和审核规则不可由候选自行改写 |

### 11.1 当前落地：证据上下文与可检验提案

`EvolutionService` 读取最多200条候选开发记录，整理器仅接受已结算回测、匹配代码指纹、
当前单一标的与周期、明确开发窗口且结束日期不晚于新任务开发窗末日的记录。数值须有限，
原始嵌套指标、源码和运行时配置不直接进入经验。最多20条经验，每条有大小上限，优先各保留
10条开发通过与未通过实验，不足时用另一类补足；同一回测引用去重。清单记录选择、排除原因和摘要哈希。
旧记录缺少这些事实时如实排除，不能补造身份或窗口。现阶段没有宣称经验具备因果效力。

自动进化提案绑定 `evolution_hypothesis`：证据引用、预期指标、变化方向、可证伪条件。
引用只能来自该任务已有Paper观察、订单样本、入选记忆或已完成开发实验。新候选及这些说明一起进入
持久事件与版本审核；描述正确性仍需实验检验，引用存在不等于因果归因成立。

相同代码、标的、周期、开发窗口和资金形成实验指纹。再次提出已入选归档中的相同实验时，必须提供
`repeat_reason`；有明确重测原因仍允许在预算内复现，因为上游数据可能修订。它是防盲目重复门禁，
不是全库数据快照去重。一次重测不构成独立支持证据。

### 11.2 后续进化必须逐项取得证据

1. **输入可靠**：修复持续采样故障，建立旧Paper会话的可追溯接入；原权益、订单、仓位和历史保持不变。
2. **诊断可靠**：对信号、退出、成本、行情状态提出可证伪假设；订单样本覆盖和费用口径不清时不作因果结论。
3. **经验有效**：固定同一模型、工具、预算和任务集，配对比较无记忆、旧记忆、新记忆；记录成功率、失败分类、调用成本、重复实验率及跨任务退化。样本不足报告未知，不拿单元测试代替模型效果评测。
4. **策略有效**：开发搜索与最终窗口隔离；策略变化和独立比较受固定门槛、人审控制。Paper最近数据已参与选题，不能把同期间历史行情宣称为完全未见信息；真实后续Paper才提供新的前向证据。
5. **能力扩展**：主动探索和组合策略分别建立预算、数据契约与回放基线；需要真实能力验证，不能因“单标的已跑通”而声明支持组合进化。
6. **研究方法自改**：未来的提示词/检索策略更新必须经过内容质量门禁与配对性能门禁，改善目标案例且不降低冻结回归集表现，再用独立留出集评估；保留旧版本可回退。当前未自动启用此能力。

验收分三级：工程不变量测试、隔离的模型配对评测、真实Paper完整循环。三个层级分别报告，
只有最后一级能证明本项目业务闭环；任何层级都不承诺持续盈利。
