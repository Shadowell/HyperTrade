...
当前配置的策略范围也适用于不可用清单行和旧延续记录；超出范围的旧告警按原生命周期 resolved。未读取到上游快照只能判为读取不可用，不能据此判定原会话身份或起点缺失。
...
且不计为一次投递尝试——webhook 配置后下个评估周期立即送达；未解决未确认
每天提醒、失败 6 小时节流持续重试）；worker 每个 tick 评估，投递失败永不阻塞扫描。

同一行在 open 期间更换条件签名也开启新事件，立即核验新内容的投递；业务成功回执持久绑定当时的签名与消息哈希，列表只为当前内容显示送达核验。旧无绑定回执保持未核验。
升级前的open旧成功回执若缺绑定，立即重新尝试当前内容，旧接受事实单独保留；失败重试仍受6小时节流，已确认记录不重投，完整绑定的同条件文案更新仍按24小时提醒。
