...
## 来源变体候选引用同窗基线封存行情（规格015） — 2026-09-26

- 配合 BitPro PR #960 的固定预热门禁：来源变体实验身份纳入父 manifest 与参数（此前同父策略的所有变体共用创建/回测幂等键）；候选回测前以基线实验相同的键创建并回测同窗基线，并引用其唯一封存快照；基线无封存绑定时候选失败关闭。连接器放行 v3 绑定与预热字段。
...
## 策略范围扩展与首个受控候选门禁回执 — 2026-09-25

- **12项运行策略来源政策核验**：生产 12 个运行策略（5个 `native_registry`: 107, 333, 378, 480, 505；7个 `db_script`: 443, 446, 499, 501, 509, 511, 513）逐一完成只读来源与变体政策核验。全部 12 项当前均有受控参数政策，且 `variant_creation_supported=true`；执行与回测能力须独立验证。
...
- **原模拟盘连续性与剩余门禁**：生产回读时12个原 Paper 实例（包含策略333的`paper_c37a0087432d4af2a90e72008b2ebaa7`）仍running，实例ID、起点和成交历史保留；`paper_review_mode`保持`human`。候选518的回测已被门禁拒绝，尚无同窗可比结果或经审核的新Paper；T009/T010继续未完成，不把范围扩展或列出阻塞计为100%。
...
- 本切片只完成受治理的接口能力。AVO尚未产生来源参数提案，也未创建真实候选、执行可比回测、更新记忆或发起新Paper；这些仍在规格008的T006b–T010中逐项验收。
- 连接器定向回归先证明缺工具路由与源码摘要泄漏，再验证来源/政策/候选身份、封存行情ID回传及无效基线请求被拒；完整`./scripts/check.sh`后端1714项、前端与Ruff/mypy通过。b999ed1经Actions35824507247部署，服务器SHA和`/api/health`一致；生产HyperTrade容器只读调用BitPro返回#513来源`selected_unverified`、3项授权参数、创建政策可用，来源与政策manifest相同。未调用变体创建。

## 回测来源与行情输入冻结部署验收 — 2026-09-23

- BitPro PR #940/cce08d60经Actions35822740342部署，服务器SHA及认证API健康已核对。异步任务入队绑定来源、配置/成本/资金/请求和包指纹；`preparing_data`首次绑定封存行情ID，恢复仅复读；无历史快照的旧任务只读展示，不用当前策略行重建执行。
- 生产独立只读/封存检查：BTC永续2026-09-20 00:00至09-21 00:00 UTC 共25根官方已收盘1H bar，快照`vbs_1c5dfa1de032cf81e348a037e93460f340f65cf245ec14349b02b9effd230e75`由实际Backtrader数据读取器取回25根，北京时间首尾为09-20 08:00与09-21 08:00。此证明数据路径和UTC边界，不是已完成回测、盈利或可比实验。
- 部署前后12个实际运行Paper的实例ID、起点和running状态逐一相同，成交数未回退。当前没有安全预热门禁和tick历史执行，来源绑定的候选明确阻断比较；Agent提案/记忆/审核与真实新Paper仍为后续任务。
...
- 发布前后12个实际运行Paper看板状态均为running，逐策略成交数未回退；最新Paper实例ID与起点逐一相同。512策略与看板为stopped，但旧实例记录仍标running，后续单独核对，不计入当前实际运行集合。
- 513已被另一项获授权的任务改为1H固定15币DB结构策略；2026-09-22的13项来源清单仅为当时快照。当前12项清单和变更边界已同步规格008，参数政策需依据新来源重审。受控变体、可信数据回测与Agent编排仍未完成，进化范围仍保持原生产策略511及人工Paper审核。
- 跨日回归纠正两项预算测试：既有用例用9月22日的固定入账时间，却以当前日读取周期预算；测试现在只在读取状态时固定同一时钟，不改生产预算计算。两项定向通过，完整`./scripts/check.sh`后端1712项、前端/静态检查通过。

## 运行策略来源与告警集成验收 — 2026-09-22

- 发布核验：3dba94f经Actions35720661870成功部署，服务器SHA与健康检查匹配。13个运行策略周期全部observed；真实BTC的1m来源使用1h基准，337个已闭合边界点读取成功。511当前来源缺口于2026-09-22T11:19:40Z取得飞书业务码0，条件签名和消息哈希均匹配，旧未绑定回执单独保留；页面为1项待处理，原部署期临时告警已恢复。
- BitPro PR #923已合并，22b67543经Actions35721180856部署成功，本地main已同步且既有未跟踪文件保留。认证API逐一返回13个来源：6个native_registry、1个native_module、6个DB静态selected_unverified；120/174标的完整保留，所有execution_verified/historical_execution_verified均为false。真实执行、变体创建和新旧Paper闭环仍待后续验收。
- ffcc451经Actions35710981546部署成功；恢复读取后真实页面只保留511一项来源缺口告警，明确历史源码/成本/执行绑定未知与10月3日只是时间下界。此前部署期间的不可读清单被误分类、越scope告警已复现，规格009修复进入本轮集成。
- 周期/小时闭合K线基准、诊断范围/契约分类、当前条件/消息绑定的飞书回执及旧未绑定回执真实重投联合全检1712项、前端15项及lint/build/Ruff/mypy通过；已发布并核验新回执。旧17:41回执不能证明后续来源缺口文案送达。
- 全部原逻辑调优的剩余实现与逐策略验收正式列入规格008：当前来源、受控变体、不可变回测、AVO参数工具和真实新旧Paper回执。BitPro来源接口完成安全复核、实际120/174标的覆盖和46项测试，已发布并逐一读取13个来源；统一受控loader和后续变体仍在开发。

## 进化告警投递事件绑定 — 2026-09-22

- 生产只读回执揭示：511当前来源缺口说明与此前“会话缺失”的业务成功回执共用一条open告警，旧投递时间被用于新条件的24小时节流，页面误显示已核验。本切片让open期间签名变化立即开启新事件；成功投递绑定条件签名和消息哈希，缺绑定历史回执及未重发的新文案均显示未核验。
- 保留同条件每日提醒、失败六小时重试、确认静默及恢复重现规则；仅模拟投递进行本地回归，不手动发送飞书，不操作Paper。告警定向36项、Ruff、格式检查和目标mypy通过；总控合并后再统一全检与发布核验。
- 升级兼容补丁补齐旧open/sent但没有内容绑定的记录：不沿用其24小时送达节流，尝试给当前来源缺口新文案投递；原回执作为未绑定历史观察保留，新尝试失败后仍守6小时重试。补丁仅用假投递做定向回归，未接触生产飞书或原Paper；总控合并后统一全检。

## 进化诊断范围与上游读取分类 — 2026-09-22

- 修复扫描中不可用清单行绕过 `strategy_ids` 的旧缺陷，并让当前预览及旧延续告警都按当前范围投影；范围外旧告警沿用原恢复规则。BitPro 部署期间的短暂只读失败不能证明原Paper历史损坏。
- 未返回快照时固定报告上游只读读取不可用及重试行动；已返回但身份或起点确实缺失时保留原阻塞。成本、成交、14天窗口、预算、研究和Paper审核门槛均未放宽。定向回归先复现三个缺陷，再通过109项相关测试；完整 `scripts/check.sh` 通过（后端1678项、前端15项、lint、构建、Ruff、mypy）。尚未推送或发布，生产验收待主线集成后核对。
- 复核补丁：不可归属的清单ID不再中止整轮或误指其它策略；已返回却未通过身份/格式校验的快照走固定契约告警，临时传输失败仍为可重试读取故障。后续只做定向与静态验证；总控在合并周期/基准补丁后统一重跑完整检查。

## 自主进化剩余项集成 — 2026-09-22

- 多任务记忆配对增加冻结矩阵、独立journal、恢复与过程指标；总控复核修复相同目标不同任务共享实验ID的问题，批次执行ID和任务范围现在进入pair哈希。真实模型效果评测尚未执行，不宣称记忆提高收益。
- 多市场读取采用类型化端口和MCP桥接，显式交易日证据、opaque策略ID、跨目标预算和续跑键隔离；非BitPro研究/Paper写明确deferred，告警/效果账本与QuantLab实机仍未完成。
- Paper独立来源核验上游PR #901已部署；最新只读核验13个运行来源均可读取，509为verified，其余12个仍有历史成本或代码绑定缺口。消费者保留这些差异，不读取明细补历史；窗口完整性仍单独判断。
- 交易日两半窗口以共享收盘边界计入隔夜跳空。联合检查暴露的market_compare异常已定位为SQLite内存StaticPool被并行工具共用同一连接，实例级事务锁修复并覆盖提交/回滚/关闭；完整检查后端1672项、前端15项及lint/build/Ruff/mypy均通过。2214187经Actions35709571112部署成功，线上511/509来源状态差异、原实例身份、scope511/human/proactive=false/revision5均已核对。
- 页面复核发现来源缺口尚未进入告警条件：已将固定来源原因加入只读资格游标及操作告警，不更改研究准入阻塞和时间下界；采样故障与来源缺口可同时显示，来源恢复才解警。失败回归后57项定向、1674项全量通过，独立复核无阻塞；告警补充发布与页面回执待核验。
- 当前目标覆盖全部运行中Paper，生产配置仍只含511。13个来源的源码、周期、资金与版本已逐一只读清点；完整7+7扫描曾因服务重启/SSH中断而未完成。当前AVO候选仍经固定模板生成，未锁定原策略参数与资金；native策略的短script_content只是说明，不可作为可执行源码。下一切片须通过BitPro真实策略解析与受控参数变体，覆盖native和DB script，并绑定不可变来源、成本和资金后回测/审核；仅扩大扫描或列出阻塞不算完成。


## 市场目标读取端口与 sessions 日历（任务分支）— 2026-09-22

- 进化扫描的清单、原策略、会话、成交与收益序列经目标只读端口接入；BitPro 使用兼容适配器，原连续 UTC 窗口和门槛保留。非 BitPro 策略 ID 可作为不透明字符串保留到扫描诊断；注册档案对 BitPro 仍要求正整数配置 ID。
- MCP 读取桥接使用真实 `McpClientRegistry` 调用 `market-evolution.v1` 规范工具，并增加来源和交易日两个只读扩展；离线 transport 夹具经过 tools/list 到同一扫描入口。真实目标服务器接入仍未核验。
- 端到端来源键按目标/策略/会话隔离预算与资格续跑；BitPro历史资格主键不变。长寿 worker 在目标切换后重建自动读取适配器；BitPro 来源错返或缺 id 拒绝，旧 fixture 补正式身份。
- sessions 7+7 口径补前一交易日收盘基准和第7日共享收盘边界；隔夜跳空进入最近窗收益/回撤，缺基准日或收盘点拒绝。此修正仅经离线合同夹具定向验证，未作真实市场接入证明。
- sessions 目标须提供目标时区、完整交易日清单与逐点交易日；最近14个已收盘交易日按7+7比较。缺失或含糊交易日、边界、版本、费用身份、分页时拒绝证据。非 BitPro 合同夹具验证读取、资格及退化决策；写端口未迁移时明确 `deferred_target_write_port`，不创建研究任务。
- 本切片未接 QuantLab 实机、规范 MCP 服务端工具或 Paper 写端口；未修改费用及审核政策。与 R8 预算来源身份合并验证后，离线 MCP/BitPro/预算联合定向通过，完整 `./scripts/check.sh` 的前端 15 项、后端 1642 项、Ruff、mypy 和构建通过；尚未发布。

## 进化告警可靠性 — 2026-09-21

- 发布核验：40950e6 经 Actions35586275985成功部署，飞书于2026-09-21T10:02:11Z返回业务码0；PR #888/BitPro83330cc2生产已核验，511六小时读取current/7点/约0.37秒，原会话、版本、起点不变，桌面及390px告警区可见。
- 收敛发现：手动诊断成功后旧告警仍依赖上一自动周期。补充只读预览投影：同政策revision、三小时内且比自动观察更新的完成诊断仅更新告警，不改研究资格或预算；旧revision不采信。完整检查1609项通过，补充发布待核验。

- 修复仅凭 HTTP 成功误记飞书送达；业务成功码、回执版本和计数持久化，旧 sent 如实标为未验证。未解决未确认告警每日提醒，失败每6小时重试且不在7天后放弃；恢复重现清除旧节流。
- 告警增加中文原因、下一步及时间条件；列表优先未解决，防止被新恢复记录淹没。诊断识别真实复现的高频采样点数上限422，保留成本/身份门槛与原Paper。
- 完整 scripts/check.sh 通过：1607 项测试，前端 lint/test/build、Ruff、mypy通过。发布和BitPro页面接入待后续核验；需求清单见 specs/001-evolution-alert-reliability/spec.md。

## 统一短期请求上下文 — 2026-09-13

- AVO 与通用 planner 接入共享 compaction.v1，完整注入后检查消息/工具 schema；原始业务事件与工具结果保留，数值序列摘要带哈希，未知副作用/必要来源/最近组不可丢弃。
- 新增独立私有脱敏快照与只含 manifest 的活动回执，支持确定性重建及所有权/尺寸/有效期检查；迁移 0043 不修改 Paper/Live 表。
- 专项测试通过，完整回归与并发主线集成、生产审计回执核验进行中。

## Paper归因证据边界（任务D）— 2026-09-13

- 新增版本化只读归因报告，绑定会话/版本/窗口/成本与证据引用；接入扫描、AVO假设和API/CLI。
- paper_evidence.v1执行流水仅支持覆盖摘要；闭合交易、入退出基准、资金费与regime缺口保持unknown，不从pnl或buy/sell推断归因。
- 上游能力部署、完整检查和只读生产核验待完成；不声明策略效果或真实14天闭环。
...
## 任务 A：持久共享研究预算 — 2026-09-13

- 主动探索与两条退化研究入口共用原子任务/额度准入，兼容既有配置，持久累计/UTC日消耗、并发、终态冷却、拒绝与恢复回执。旧Paper及原开关不修改。
...
## 不可变组合研究基线 — 2026-09-13

- 新增只读组合 manifest / 等权持有基线 / 自筹再平衡费用回放与管理员入口。冻结成员版本、内容、权重和成本，跨窗口、缺数据、成员漂移均失败关闭；内容寻址记录支持并发和中断重放，完整源序列不落库。
- 真实 BitPro 回测500的小时权益可读，但现有返回没有逐期间费用和换手；总值保持未知，研究比较为 needs_data，不把缩放净权益指数称为策略实盘/模拟执行。
- 专项与完整验证、迁移及槽位6部署核验进行中。费用完整正向样本、真实观察时间门槛仍未完成；不修改任何Paper历史或Live。
...
- 原任务arc_f6e91935e24c的创建被明确拒绝后，以只读唯一名称查询证明0策略记录，追加恢复事件，预算未清零。随后真实开发回测499、最终回测500通过，worker以agent_policy身份自动批准一次，创建独立策略509 / paper_d6a178c0c7ff474b918e053a66b05546。真实快照running、1笔成交、起点2026-09-13T11:34:28.656807+00:00；原501仍在原实例/原起点running、5笔成交。
- 新候选/模拟盘冻结research_costs.v1：taker5/maker2/slippage1 bps，funding=strategy_defined_or_not_modeled，不宣称资金费已完整建模。最终窗口2026-07-14至2026-09-12，本次是固定基线技术验收，不是新Alpha有效性证明。
- CLI真实流跨越Agent评审、配置启动到Paper观察，并收到后续观察事件，未自动退出；浏览器显示“Agent自动评审已通过/模拟盘正在运行”，没有人工批准动作。退出观看不停止服务器任务。
- 已验证研究→回测→自动评审→真实独立Paper这一段；真实7+7退化→再次优化→自动晋级仍待完整运行数据，不能声明已全部跑完。Codex定时任务已删除，循环由HyperTrade worker负责。

- 2026-09-13 真实自动流程验收发现创建适配器把永续symbol降为现货格式，成本冻结接口正确拒绝且未创建策略。冻结成本的创建现在保留永续身份并显式传swap；旧未冻结请求保持兼容。全量check.sh通过（1353 passed / 1 xfailed），原任务arc_f6e91935e24c预算及候选保留，发布后按明确无写入证据恢复。

- 2026-09-13 自动评审复核：同窗比较将新旧最大回撤统一为幅度，避免负号格式把更大回撤误判为改善。正负格式回归重现后修复，完整check.sh通过（1351 passed / 1 xfailed）。

## 区分历史缺口与近期采样 — 2026-09-13

- 受阻策略保留原阻塞原因，并追加一次有界的最近6小时只读检查；报告近期采样正常、过期、缺口或无法确认及下一步。严格绑定原会话/版本、成本和分页，未知读取不冒充采样器停机；缺正式会话只提示核对，不创建或迁移历史。
- 近期正常不满足或替代14天观察门槛，研究创建与审核门禁不变。现有页面reason同时展示两类信息，完整结构随调度诊断回执持久化。
- 实际只读核验：333近期6小时有6个权益点但历史窗口仍有缺口；501曲线接口报告缺少taker fee元数据，尚需上游核对。完整scripts/check.sh通过（1335 passed / 1 xfailed），前端、Ruff及mypy通过；部署后只读复验随后执行。

## 全自动Paper评审授权与实现 — 2026-09-13

- 用户明确要求Agent自己评审、自己执行，取代此前逐版本人审要求，范围仅Paper。新增持久模式、确定性证据复核、agent_policy审计身份及自动启动调度；保留原实例、审批版本和未知效果不重试。
- CLI自动评审不断流，流水线区分Agent和历史人工身份；BitPro模式UI配套中。完整scripts/check.sh通过（1350 passed / 1 xfailed），覆盖政策权限与系统门槛、评审、成本证据、禁用、资金、版本、重复启动、预算结束及未知结果；部署并启用后继续真实验收。


## 开发假设评估与生产接手核验 — 2026-09-13

- 开发回测后按所引用实验核对标的、周期、实际开发窗口、资金和完整回执，记录指标方向 observed / not_observed / mixed / unknown；缺失、非有限或不可比证据不打断已完成回测，且不能形成成功判断。后续有界记忆保留评估摘要，不把它视为因果、样本外或审批结论。
- 回归覆盖提案→开发→事件序列化→后续记忆召回，并保留预算及无Paper写入边界；完整scripts/check.sh通过（1322 passed / 1 xfailed），前端检查、Ruff和mypy通过。
- 现场确认40e4684部署成功且服务器SHA一致；真实记忆SOL 3条、BTC 7条。策略501原实例paper_dd3f7bcca6e64a1f802004ef7ab66791仍running，快照5笔成交、权益99.96665。此为时点只读证据，真实14天反馈、新版本人审与并行Paper仍未完成。

## 论文驱动的可靠进化 — 2026-09-13

- 对照 GEPA、ACE、MemRL、HarnessEvolve、DGM 原论文更新架构61第11节，区分机制借鉴与实证效果；不引入生产自改裁判或自动批准能力。
- 自主进化接入开发记忆的标的/周期/代码/窗口门禁、有界正反例与摘要清单；旧数据不补造，原Paper不变。提案需引用现有证据和可证伪目标，重复归档实验需说明重测原因并计预算。
- 57项专项回归与完整check.sh通过（1302 passed / 1 xfailed）；实际Codex/GPT-6 Astra完成3项隔离协议验收：引用证据提案、非可信记忆不改变指定标的参数、缺证据诚实停止。仅使用合成协议输入，外部工具派发为0；不代表配对效果评测或真实业务闭环。生产部署和历史数据只读核验随后执行。

## 持续闭环验收 — 2026-09-12

- 反馈循环补验发现：已得到明确失败回测结论的调优子任务仍被当作进行中，永久阻止后续检查。现按实际回测结果判定已结束的负向研究，仅在下一UTC日重新检查；未知副作用、数据不完整和提前停止仍阻断。保留原任务与Paper，不伪装为成功。16项反馈回归及完整scripts/check.sh通过（1251 passed / 1 xfailed）。

- 新人审路径接入原子配置/启动专用接口，提交代码SHA、review_hash、参数/执行策略与会话版本；核对guard_version并禁止旧接口降级。53项接口/审核定向回归通过，完整scripts/check.sh为1246 passed / 1 xfailed。BitPro PR #820已部署，专用接口无效绑定生产负向验收拒绝且未新增Paper会话；调用端部署后开始真实研究。
- 下一阶段按现有门槛创建真实研究候选，交用户审核后跟踪真实观察和再次审核；保留原实例，未完成不得宣称闭环。

## Paper启动交接纠偏 — 2026-09-12

- 生产仍为28个needs_operator任务，无待审核或paper_observing任务。发现启动器将真实paper_...会话ID转为整数，且复用策略ID前未核对源码。
- 分离策略/会话标识，核对配置版本、启动回执与原运行历史；53项启动/审核/反馈相关回归通过，完整scripts/check.sh为1242 passed / 1 xfailed。生产源码与只读快照确认真实标识合同；部署后只读核验待执行。
- 真实合格候选、人审后启动、14天反馈仍待验收；不降低门槛，不操作存量模拟盘。

## 恢复语义与无动作重试 — 2026-09-10

- 原任务恢复后模型仍把旧stop回执视为结束，只返回总结；恢复状态未作为新操作者消息进入模型历史。补充幂等恢复指令，保留原实验、最终窗口和预算，未结算工具响应前不插入用户消息。
- 无工具调用的文字答复最多自动重试2次，计入正常模型预算；连续3次无动作或预算耗尽仍明确停止。活动流区分模型说明与实际调用，并展示公开文字答复；不暴露推理或完整上下文。
- 44项恢复/进度定向回归通过；完整scripts/check.sh为1232 passed / 1 xfailed，Ruff/mypy及前端检查通过。恢复指令只存在于新事件，旧历史不补造消息。原任务已用6/20模型调用、1/5候选，未消费最终窗口，没有Paper审批；上线后复验。
...
- 页面默认Codex完成10次模型请求，3个候选计数自动更新；旧超额开仓被拒绝，新候选122次开仓资金核对无越限。
- 第三候选开发达标；同名策略重复创建已修复，显式对账恢复后最终结果481收益-0.95686%、回撤3.9255%、23笔，未达标、没有启动Paper。
- 页面按当前动作和研究→审核→模拟组织，开发/最终结果分开；完整14天闭环仍未验收，详见docs/qa/2026-09-10-bitpro-agent-e2e.md。
- BitPro发布遇484退出超时，检查点+20笔账本重放与原账户一致后恢复20实例；后续完整封存成功，退出等待修复纳入PR #812。

## 最终验证复用已有策略 — 2026-09-10

- 真实复验 arc_1f27c5576d40 第三候选开发达标，进入最终验证时重复创建同名策略触发 BitPro 外键保护，尚未派发最终回测。
- AVO 继续使用开发回执中的策略ID；SelfTest先读ID和源码并核对一致性，避免重复创建或覆盖。源码不符即拒绝继续。
- 完整检查1218 passed / 1 xfailed；新增开发→最终验证集成回归通过，确认同候选仅创建一次策略、执行两次回测。
- 同任务恢复须先确认未派发最终回测，保留已计入的动作预算与原审批边界。

## 端到端缺陷修复 — 2026-09-10

- 默认 Provider/model 改为 Codex/GPT-6 Astra，测试隔离本机 Codex 登录文件。
- 新生成策略默认1倍杠杆、单次目标保证金20%净值，拒绝回执不造持仓；每Bar核对实际持仓，资金策略进入审核包。
- 本地回放对齐名义金额、保证金与回执协议，修复重复乘杠杆。完整检查1216 passed / 1 xfailed，6类生成代码通过BitPro结构冒烟。
- 配套 BitPro 回测保证金约束、计数修复和页面信息简化；生产联合验收待完成。

## BitPro 页面端到端验收 — 2026-09-10

- 真实页面任务 arc_797979d72aac：默认 DeepSeek 402 阻断；CLI 显式恢复 Codex 后完成3候选/3回测/11次模型请求，全部未达门槛，未消费最终窗口，无Paper操作。
- 整体 FAIL：发现默认模型入口、资金/仓位约束、顶部计数、开发证据展示和回测引用问题。100U回测接受约500U保证金开仓并出现负权益，需修复后重新验收。
- 完整检查1213 passed / 1 xfailed；详见 docs/qa/2026-09-10-bitpro-agent-e2e.md。人工批准及真实14天闭环未执行。

## Paper 7+7 反馈实现 — 2026-09-10

- 新任务配置、窗口比较、版本/数据门禁、持久父子关联和去重已接入现有观察 worker；子任务受同策略族参数约束，最终进行原策略/候选同窗比较并再次人审。
- 定向回归覆盖 10 点边界、任意指标触发、数据缺口、子任务创建中断恢复、原 Paper 连续运行、真实回测接口形状与审核包比较门禁。合成样本仅用于单元测试。
- 完整 scripts/check.sh 通过：1213 passed / 1 xfailed，前端 lint/test/build、Ruff、mypy 通过；原策略比较后的超时不派发第二次回测已回归。
- 生产清点 25 个研究任务，paper_observing 为 0；当前运行的外部策略 107 尚无不可变 Paper 会话，paper_snapshot 如实拒绝。不能将其累计 dashboard 数据用作真实 7+7 验收。

## AVO 预算语义修复 — 2026-09-10

- 已复核真实事件：服务端候选用量为 1/2，模型把同一历史候选重复相加。请求现提供权威剩余额度及已计数候选 ID；本次模型请求计入快照，恢复旧任务刷新说明。
- 完整 scripts/check.sh 通过（1200 passed / 1 xfailed），定向回归 19 项覆盖两次候选后的余额、开发/最终回测保留与请求快照一致性；真实多轮修订见下方回执。

- 新任务 `arc_b8c4cf4886c3`：GPT-6 Astra 读取基线回测 472（策略 486，8/32 均线，Sharpe -0.2722、回撤 135.4505%、41 笔）后，自主提出同族 16/64 均线修订，回测 473（策略 487）取得 Sharpe 0.4214、回撤 70.0369%、22 笔；两者均未达门槛，均为 2026-03-13 至 2026-07-10 开发窗、100 USDT 初始资金。
- 8 次请求用尽时系统正确暂停；通过幂等 continue 明确追加 2 次（总 10），候选/回测上限仍为 2/3。模型完成第二次开发回测后正确识别候选余额 0、开发回测余额 0、最终保留 1，并自行 stop。实际消耗 10 请求、2 候选、2 回测；最终窗口未揭示、0 Paper 事件。
- 这次证明在指定基线与同策略族范围内，真实模型按实验反馈修订参数并重测；不证明盈利、因果归因或任意策略结构演化。当前 propose 不接受显式止损/止盈覆盖，模型从拒绝反馈恢复并检查默认值；该限制消耗了额外请求，后续需完善参数发现。7+7 Paper 反馈仍未实现。

## AVO 真实小预算验收 — 2026-09-10

- 部署 `49a3b22` 后，同一任务 `arc_0ec7193c1421` 经 CLI 恢复并使用账号可用的 GPT-6 Astra；7 次请求中 4 次成功回复，前三次为 DeepSeek 402 与 Codex 旧模型 400（含一次记账诊断）。没有清零预算或购买额度；只追加 1800 秒排障时间，候选/请求/回测上限仍为 2/8/3。
- 模型实际选择 inspect → propose → develop → stop；BitPro 策略 485、开发回测 471，开发窗 2026-03-12 至 2026-07-09，40 笔交易、Sharpe -0.2721、最大回撤 135.4505%，未通过。最终窗口未消费、0 Paper 事件。异常的大回撤也意味着这些回测数字不能被当作可靠盈利结论。
- BitPro 页面已显示第 24 个任务、开发实验及停止理由；“批准模拟盘”禁用，原有 23 个任务保留。真实 PostgreSQL 跨连接互斥已通过；完整检查为 1199 passed / 1 xfailed。
- 验收边界：证明真实模型/工具/回测反馈和失败停机路径；没有证明成功候选最终验证、真实人工批准或自主修订第二个候选。模型停止理由中把已用候选重复计算（实际为 1/2），后续需改进预算语义并再验收多轮修订；不可把这次停止描述为系统预算耗尽。7+7 Paper 反馈仍属下一切片。

## AVO 任务级 Provider 选择与计费阻塞 — 2026-09-10

- 真实 canary `arc_0ec7193c1421` 在第一次模型请求收到 DeepSeek HTTP 402；系统停在 needs_operator，0 候选、0 回测、0 Paper。
- 新增 start/continue 的任务级 Provider 选择，不改全局默认；只允许在任务已停止且尚无候选时更换，避免混淆版本来源。切换会明确废弃尚未执行的旧动作；恢复旧动作时仍保留原请求的模型身份。
- 验证：完整 scripts/check.sh 通过，1199 passed / 1 xfailed；覆盖不修改全局默认、已生成候选禁止切换、旧动作作废和恢复时的原模型溯源。
- Codex 配置 gpt-5.4 返回 HTTP 400（ChatGPT 账号不支持）；按当前客户端版本查询账号模型列表后确认 GPT-6 Astra 可选。新增任务级 --model，模型改选遵循相同停止/无候选限制及幂等绑定，不改全局模型。候选/模型/回测上限保持 2/8/3。
...
- 核对 BitPro `strategies.name UNIQUE` 与保存时的 `INSERT OR REPLACE`，新研究不仅隔离幂等键，
  也使用带任务/代码哈希的 canonical 策略名，避免同目标覆盖旧策略；同任务重试仍保持稳定名称。
- 回归用例先复现“键不同但名称相同”，再验证名称与键均跨任务隔离；没有操作现有策略数据。
...
- 根据产品要求与 AVO 原论文，扩充架构 61：自主变异内循环嵌入 ARC 领域主线，复用版本/证据、
  沙箱和工具治理；区分开发反馈与独立样本外，保留逐版本人审、新旧 Paper 并行及预算约束。
- 明确单次 Provider 提案不等于 AVO，要求真实模型反馈修复轨迹；结构演化与运行时自修改不随本轮激活。
- 本次仅文档修订。前次部署 `34352447455` 在 PostgreSQL 依赖阶段失败；本次文档提交跳过 CI，
  不再次触发已知阻塞的服务部署，不宣称数据库已恢复。
...
- 2026-09-12 CLI流式与交互界面：实现SSE事件续接、默认跟进、显式detach、完整活动日志、证据/逐版本审核/预算交互；完整check.sh为1264通过、1项预期失败，补充后的7项流式/交互专项及类型/lint检查通过；生产验证待发布。新增目标：BitPro产品内每小时扫描模拟盘、历史订单和研究记忆，主动诊断并发起受控AVO进化，原策略保留、逐版本人审；后续独立切片实现，不用Codex定时任务代替产品调度。

- 2026-09-12 产品自主进化：持久开关、小时调度/只读诊断、逐实例身份/版本/7+7数据约束、当前会话成交样本、长期开发回测记忆、同源去重与冷却、基线复现及同窗比较已实现；完整检查1280通过、1项预期失败，补充21项迁移/调度/终端交互回归通过。CLI流式已部署并验证真实任务回放；补充修复证据面板退出键。服务器磁盘空间已通过清理可重建缓存恢复；产品调度生产联调进行中。

- 2026-09-12 自主进化真实诊断发现旧Paper REST筛选参数错位：总览/事件/曲线须将逻辑strategy_id映射到数字instance_id，新版paper_snapshot保持真实会话语义。26项适配器回归与全量check.sh（1284通过、1项预期失败）通过；未使用身份错配数据生成研究。

- 2026-09-12 批量Paper诊断限流修复：专用只读客户端串行控制请求节奏、缓存30秒健康检查、对429有界退避；不缓存交易数据，不重试写操作。扫描逐策略持久化进度，7+7反馈同样复用此读取路径，避免瞬时请求打满服务配额。30项专项与完整check.sh（1286通过、1项预期失败）通过。

- 2026-09-12 部署空间复发修复：依赖安装层移到源码COPY之前，uv缓存使用BuildKit缓存挂载，应用代码改动复用依赖层；API/worker/sandbox/CLI顺序构建，减少并发解包峰值。保留原运行容器与数据；同步排除last_deployed_sha，失败保留旧部署凭据。全量check.sh通过，新Dockerfile已在服务器通过独立Compose构建，生产发布继续。

- 2026-09-12 工作流观测语义：Paper启动后保持模拟观察active，不显示全部完成；SSE继续发送观测事件，CLI不因进入Paper阶段自动结束观看。人工审核完成提示读取实际审批回执。27项流程/流式回归与全量check.sh（1287通过、1项预期失败）通过。

- 2026-09-14 可插拔市场目标与自进化通用化（Phase 1，用户指令合同）：新增 `hypertrade/targets`——`market_target.v1` 目标档案（venue/日历/证据契约/成本政策来源/能力开关）、进程级注册表（`MARKET_TARGET` 设置决定活跃目标，未知目标拒绝并列出已注册清单）、BitPro 内置目标（惰性适配器工厂）、`market-evolution.v1` 通用 MCP 契约（七个规范工具 + `McpContractClient` 按 `tools/list` preflight 校验缺项，QuantLab 类平台接入路径）与 Phase 2 类型化端口（`ports.py`）。`EvolutionConfig`：`enabled` 默认开启（仅影响新建配置）、新增 `target_id`（配置期校验已注册）、`meta_tuning_enabled`（默认 True，仅建议回执）与 `meta_tuning_auto_apply`（默认 False）；进化循环客户端（含 tick 二次身份复核）经注册表解析；`readiness()` 窗口天数/时区由目标日历参数化（continuous/UTC/14 天与旧行为逐字节一致）。离线元学习 `arc/meta_tuning.py`：只回放已结算 7+7 观测（周期账本冻结当轮配置与窗口值），阈值=观测 p90（下限 max(5pp,p50)、上限 20pp、<12 条不调参），单步 ≤3pp、每日一次（`tune_YYYYMMDD` 回执幂等），自动应用经 configure 修订审计（操作者 `hypertrade:meta-tuner`）；worker `arc_meta_tuning_loop`（每小时 tick）与 `GET /evolution/tuning` 只读报告。归因见证式升级：`costs`/`long_short` 仅在上游 `coverage.fields` 双页 `observed` 且条目数值齐备时点亮，`source_field_states` 三态；当前上游全部 unknown，线上行为不变，既有"费用不得由裸 PnL 推断"契约测试保持。清理：`_memory` 死返回值与重复活跃/阻塞计算删除，行为不变。验证：新增 `tests/test_market_targets.py`（11）、`tests/test_meta_tuning.py`（8）、归因新增 3 项，相关套件全绿；ruff/mypy 通过；文档新增架构 62、用户指令合同、spec 章节与索引。Phase 2（未交付）：`_scan` 读取面/证据序列/成本词表/Paper 供给写入端口化、sessions 日历交易日窗口、BitPro 服务端规范工具名、QuantLab 实机接入。

- 2026-09-14 自进化加固三件（用户指令合同《自进化加固》，按确认的优先级顺序）：① 效果账本 `arc/effectiveness.py`（`evolution_effectiveness.v1`）——只统计进化循环发起任务（evolution_context/feedback_parent）的周期分布、任务进度、候选/开发/最终运行与通过、基线同窗对比胜率（无效对比单列不充数、样本不足不给胜率）、Paper 决策、已结算 Outcome、候选/模型调用/回测成本与逐来源战果；`GET /evolution/effectiveness`。② 数据缺口告警——`readiness` 每个 blocker 标注 resolution（time/operator）、延续记录带 attention_required；新账本 `arc_evolution_alerts`（迁移 0046）三规则：operator 阻塞立即告警、证据无法构建且无预计资格时间先跟踪超 72h 升级、连续 3 周期 error 告警（critical）；条件消失自动 resolved、ack 后同条件不再打扰、投递复用 FEISHU_WEBHOOK_URL（未配置只记台账、失败 6h 节流重试 7 天放弃、投递失败不阻塞扫描）；`GET /evolution/alerts` + `POST .../ack`；worker 每 tick 评估。③ 基准相对退化——默认口径改为策略 7+7 对比其标的同窗买入持有（边界容差=周期长度，`market_klines` 构建）；大盘下跌不再冒充退化；15m 超单页上限/拉取失败/错位/多标的均回退绝对口径并在 window 载荷标注 benchmark.status；`EvolutionConfig.degradation_basis` 可一键回退 absolute，绝对口径 reasons 与行为不变；反馈子任务同样传递候选标的/周期。测试：新增 23 项（效果账本 5/告警 9/基准 9，含迁移建删表），相关套件 109 项与进化全套件零回归，ruff/mypy 通过。后续小切片：BitPro 面板接入 effectiveness/alerts 的代理与展示；首个真实闭环完成后用账本做复盘。

- 2026-09-14 自进化加固生产核验（部署 cc7a002，run 34806059745 success）：① 效果账本 `GET /api/v1/arc/evolution/effectiveness` 实测 45 个周期（42 no_action + 3 preview_complete）、missions_total=0（尚无闭环任务）、`baseline_win_rate=null`（无有效对比时不虚报）、causal_conclusion=not_established。② 告警账本首轮运行即产出 7 条真实告警：107（operator，含 sampling reason=session_identity_missing；107 已恢复 running 但无模拟会话）、446/501（operator，读取类）；333/342/378/443 为 `evolution_evidence_stalled` tracking（72 小时跟踪期，与 9/3-9/4 缺口待 9/18 滚出的已知状态一致）；296 暂停后无告警（陈旧守卫生效）；`GET /evolution/alerts` 冒烟通过。投递均 `skipped_no_webhook`——**HT 生产 env 尚未配置 FEISHU_WEBHOOK_URL，告警当前仅入账本**，配置后自动开始推送（失败节流重试）。③ 配置已生效：`degradation_basis=benchmark_relative`、`target_id=bitpro`；带基准的 window 载荷将在策略通过证据门槛（预计 9/18 后）的下一轮扫描出现。人工待办：处置 107/446/501 的 operator 阻塞（107 建治理性 Paper 会话或再暂停）。

- 2026-09-14 告警投递启用（f3028fa，run 34807570556 success）：按用户指令复用 BitPro 同一飞书 webhook——已从 `/opt/bitpro/backend/.env` 复制 `FEISHU_WEBHOOK_URL` 至 `/opt/hypertrade/.env`（哈希一致、留 .env.bak 备份）。配套修复：`skipped_no_webhook` 不再计为投递尝试（含历史行豁免），webhook 配置后下一 tick 立即补送而非等 6 小时重试窗。核验：worker 已读取新 env；存量 3 条 open 告警（107/446/501）投递成功（delivery_result=sent、delivered_at 已置）——用户飞书群应已收到三条「[HyperTrade 进化告警]」；333/342/378/443 维持 tracking，若 9/17 前未恢复将在 72 小时点升级推送（预计 9/18 缺口滚出后自动解决）。107 全名：`[合约][1H][CTA] TradFi半导体 · EMA5/20趋势跟踪激进版 · 100U`（running，无模拟会话）。

- 2026-09-14 组合策略自主进化（用户指令"进闭环"延伸）：移除扫描闸门对多标的策略的拒绝（原"组合策略已识别退化；同窗比较仅支持单标的"），组合策略与单标的一样进入退化诊断、告警与研究触发。落点：universe 新增 `candidate_symbols`/`declared_symbols`（组合必须显式声明全集合、单标的来源不得扩集合、多标的 scope 不静默取 index 0）；扫描的基线 spec/`goal.symbols` 携带全量标的；AVO propose 新增 `symbols` 数组并强制候选保持源集合（`enforce_source_scope`）；self-test 以全量标的 validate/create、回测 `symbol=None` 交由 BitPro 从 `trade_symbols` 推导整篮子输出聚合指标（codegen 模板本就按 `self.symbols()` 循环）；Paper 评审包/reviewed 配置绑定全量 symbols；组合基准=成员等权买入持有合成（任一成员失败即整体回退并标注）；研究记忆按"与任一成员同源"过滤；命名沿用 BitPro 组合标签（前 3 成员+等N）。测试：新增 `tests/test_portfolio_evolution.py` 13 项（含 107 形态的 4 标的扫描触发研究、self-test 全篮子、合成基准与相对触发、保集合守卫、记忆 scope），既有 arc/evolution/avo/记忆/告警套件全绿。诚实边界：结构变异（增删标的）仍留后续合同，组合进化当前为参数邻域；实测触发路径要等 107 的证据窗口（预计 9/18）通过后才在真实策略上首次走通。

- 2026-09-14 文档刷新（用户指令：重新写架构设计文档和 README）：README 中英文全量重写——修正失效徽章（11→1592 项测试、React 18→19、TypeScript 5.0→5.9、FastAPI 0.104+→0.122+，删除代码中已不存在的 TanStack Query/Recharts 表述），以真实生产路径重写概要（受治理 Mission 运行时、AVO 自主研究循环、默认开启的自主进化引擎、可插拔市场目标、组合一等公民、缺口不掩盖的诚实性边界），快速开始与 API 示例按现行接口核对（`research_mode` 默认 avo、Idempotency-Key、evolution effectiveness/alerts/tuning 面），移除已删组件段的历史宣传口吻。架构 33《系统架构》全量重写（2026-07-21 快照 → 2026-09-14 快照）：新增 AVO 研究循环、自主进化引擎（退化口径/预算准入/延续记录/自动评审/元学习/效果账本与告警）、可插拔市场目标三章；运行时分层与部署模型更新（13 个 worker 循环、46 个迁移）；安全边界表新增"自主进化"行（预算+证据+评审门禁、告警无审批权）。00 架构总览与文档中心索引同步（补 60/61 条目、33 描述刷新）。验证：./scripts/check.sh 全绿（1593 项测试）。校验过程中暴露并修复一个告警锚点缺陷：停滞 72 小时升级窗口与投递放弃窗口原锚定行插入时钟（`created_at`），现改为锚定 payload `first_seen_at`（循环自身时钟）；条件 resolved 后重现将开启新的跟踪窗口，而不是继承旧事件年龄立即升级（新增回归测试覆盖重现语义；生产中两时钟同源，行为变化限于重现事件与时钟偏移场景）。
# Paper 清单周期与市场基准修复（独立任务分支）— 2026-09-22

BitPro运行清单顶层timeframe缺失会跳过相对基准；本切片从已有身份匹配dashboard/配置投影原周期，并用337根有界1h闭合K线建立同窗基准。缺失、冲突、重复、越窗或缺口保留显式absolute fallback；原Paper、预算与生产scope未变。定向104项通过，`./scripts/check.sh`（含全量1689项）通过；独立分支尚未发布。
