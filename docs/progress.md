...
- Cleaned default CLI/Rich report rendering so low-value citation sections and poor terminal emoji/keycap glyphs are removed from displayed Agent reports while preserving the source-backed structured sections.
...
- `./scripts/check.sh` -> frontend install/lint/test/build passed; ruff, mypy, pytest passed with 153 tests.
