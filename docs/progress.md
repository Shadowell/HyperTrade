...
- 交易日两半窗口以共享收盘边界计入隔夜跳空。联合检查暴露的market_compare异常已定位为SQLite内存StaticPool被并行工具共用同一连接，实例级事务锁修复并覆盖提交/回滚/关闭；最终完整检查后端1672项、前端15项及lint/build/Ruff/mypy均通过，消费端发布待核验。
