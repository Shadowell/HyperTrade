...
test("renders harness observability from live overview", async () => {
  ...
  expect(screen.getByText("模拟盘运行")).toBeInTheDocument();
  ...
  expect(screen.getAllByText("实盘审批").length).toBeGreaterThanOrEqual(1);
  expect(screen.getByText("loi_live")).toBeInTheDocument();
  expect(screen.getByText("ETH-USDT-SWAP 买入 0.01")).toBeInTheDocument();
  expect(screen.getByText("风控：通过")).toBeInTheDocument();
  expect(screen.getByText("行情工具")).toBeInTheDocument();
  expect(screen.getByText("记忆管理")).toBeInTheDocument();
  expect(screen.getAllByText("智能体评测").length).toBeGreaterThanOrEqual(1);
