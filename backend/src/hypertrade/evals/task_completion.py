...
class OperatorTaskCompletionSuite:
    ...
    def evaluate(
        ...
        checks: dict[str, bool] = {
            ...
            "no_forbidden_decision": response is not None
            and all(
                fragment.casefold() not in response.decision.casefold()
                for fragment in case.forbidden_decision
            ),
...
def _case_from_payload(payload: object) -> TaskCompletionCase:
    ...
    return TaskCompletionCase(
        ...
        forbidden_decision=_strings(expected, "forbidden_decision"),
