...
class MissionRuntime:
    ...
    async def run(self, mission_id: str) -> MissionProjection:
        with _TRACER.start_as_current_span("mission.run") as span:
            ...
            if mission.status == MissionStatus.DRAFT:
                ...
                await self.store.save_plan(mission_id, plan)
                mission = await self.store.get(mission_id)
                mission = await self.store.transition(
                    mission_id,
                    expected_version=mission.version,
    ...
    async def _execute(self, mission: MissionProjection) -> MissionProjection:
        while mission.status == MissionStatus.RUNNING:
            ...
            await self.store.complete_attempt(attempt.attempt_id, observation)
    ...
    async def _apply_safe_point(self, mission: MissionProjection) -> MissionProjection:
    ...
    async def _request_replan(
        self, mission: MissionProjection, previous: PlanV2, request: ReplanRequestV1
    ) -> MissionProjection:
        ...
        return await self._activate_replan(mission, previous, request)
    ...
    async def _activate_replan(
        ...
        self._validate_plan(mission, plan)
        await self.store.save_plan(mission.mission_id, plan)
        mission = await self.store.get(mission.mission_id)
        return await self.store.transition(
            mission.mission_id,
            expected_version=mission.version,
    ...
    def _attempt_plan_version(
        ...
        return plan.version == 1 or attempt.step_id in plan.diff.kept
    ...
    def _criteria_satisfied(
        ...
        *,
        ...
        source_count = len({ref for item in observations for ref in item.source_refs})
        artifact_refs = [ref for item in observations for ref in item.artifact_refs]
        result_fields = Counter(key for item in observations for key in item.result)
        for criterion in mission.success_criteria:
            if criterion.kind == "all_steps_validated" and not observations:
                return False
            ...
            if (
                ...
                return False
            if (
                criterion.kind == "observation_field"
                and str(criterion.expected) not in result_fields
            ):
                return False
        return True
